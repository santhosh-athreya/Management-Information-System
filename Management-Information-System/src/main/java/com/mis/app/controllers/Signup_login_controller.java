...
public class Signup_login_controller {
	...
	public ResponseEntity<LoginResponse> login(HttpServletRequest requestHeader, @RequestBody LoginRequest request)
		...
		logger.info("Inside login controller username: {}, password: {}"+ request.getUserName(),request.getPassword());

		LoginResponse response = service.login(request.getUserName(), request.getPassword());

		if (response == null) {
			throw new MyCustomException("Login failed. Possible cause : incorrect username/password",
	...
	public ResponseEntity<?> signUp(HttpServletRequest requestHeader, @RequestBody SignUpRequest request)
			throws Exception {
		...
		try {
			...
			// Return Created Status code with Created User URI - User/{id} - best Practice
			 ...
			 * 
			 * // construct model with user response EntityModel<User> model =
			 * EntityModel.of(user);
			 * 
			 * // Plain Request LoginRequest req = null;
			 * 
			 * // Build Link - attach the method WebMvcLinkBuilder links =
			 * linkTo(methodOn(this.getClass()).login(requestHeader, req));
			 */
			// add link
